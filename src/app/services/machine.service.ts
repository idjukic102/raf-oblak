...
import { CreateMasina, Masina, StanjeMasine } from '../model';
...
export class MachineService {
  ...
  addMachine(machine: CreateMasina): Observable<Masina[]> {
    const nextId =
      machines.length ? Math.max(...machines.map(m => m.jiBroj)) + 1 : 1;
  ...
  setTimeout(() => { m.stanje = StanjeMasine.Ugasena; }, halfMs);
  setTimeout(() => { m.stanje = StanjeMasine.Upaljena; }, totalMs);
